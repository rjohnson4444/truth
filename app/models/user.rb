class User < ActiveRecord::Base
  ...
  def friendship_status(user_2)
    friendship = Friendship.where(user_id: [self.id, user_2.id], friend: [self.id, user_2.id])
  ...
  def friendship_relation(user_2)
    Friendship.where(user_id: [self.id, user_2.id], friend: [self.id, user_2.id]).first
  end
